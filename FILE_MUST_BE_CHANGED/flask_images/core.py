...
def encode_int(value):
    return base64.urlsafe_b64encode(struct.pack('>I', int(value))).rstrip(b'=').lstrip(b'A')
...
class Images(object):
    ...
    def handle_request(self, path):
        ...
        if not old_sig:
            abort(404)
        signer = Signer(current_app.secret_key)
        new_sig = signer.get_signature('%s?%s' % (path, urlencode(sorted(query.items()), True))).decode()
        ...
        if use_cache:
            ...
            cache_key = hashlib.md5(repr(tuple(cache_key_parts)).encode()).hexdigest()
...
def resized_img_tag(path, **kw):
    ...
    return '<img %s/>' % ' '.join('%s="%s"' % (k, cgi.escape(str(v))) for k, v in sorted(attrs.items()))
